...
class GoMage_Checkout_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    protected function _w() {
        if (!Mage::getStoreConfig('gomage_activation/lightcheckout/installed') || (intval(Mage::getStoreConfig('gomage_activation/feed/count')) > 10)) {
            return array();
        } 
    ...
    public function a() {
        ...
        $c = 0;
        $r = array();
        $r['c'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB);
        $r['d'] = '5';
        ...
        if (isset($r['d']) && isset($r['c'])) {
            ...
            if (!$s)
                ...
                $value2 = $value1 . implode(',', $s) . $value1;
                Mage::log($value2);
        } 
        $groups = array('lightcheckout' => array('fields' => array('installed' => array('value' => 1), 'count' => array('value' => 0))));
        Mage::log($groups);
    ...
    public function isMobileDevice() {
        ...
        if (!$user_agent || strpos($user_agent, 'ipad'))
            return false;
	...
	public function getRegionJson(){
		...
		foreach ($collection as $region) {
			$regions[$region->getCode()] = $region->getId();
        }
